    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(icon: const Icon(Icons.print_outlined, color: Colors.black), onPressed: () {}),
        ],
      ),
            ...
            child: Column(
              children: [
                Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    const Icon(Icons.water_drop, color: Colors.black, size: 24),
                ...
                _infoItem(Icons.person_outline, 'Khách hàng', customer.name, 'ID: ${customer.code}'),
                const SizedBox(height: 20),
                _infoItem(Icons.location_on_outlined, 'Địa chỉ', customer.address, null),
                const SizedBox(height: 20),
                _infoItem(Icons.calendar_today_outlined, 'Thời gian', DateFormat('dd/MM/yyyy HH:mm').format(bill.date), null),
                
                const Padding(padding: EdgeInsets.symmetric(vertical: 25), child: Divider(height: 1, color: Colors.grey, thickness: 0.1)),
                
                _billDetail('Chỉ số cũ', '${bill.oldReading} m³'),
                _billDetail('Chỉ số mới', '${bill.newReading} m³'),
                _billDetail('Tiêu thụ', '${bill.consumption.toInt()} m³', isBlue: true),
                _billDetail('Đơn giá', '12.000đ'),
                _billDetail('VAT (10%)', format.format(bill.vat)),
                ...
                Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Text(format.format(bill.totalAmount), style: const TextStyle(color: Color(0xFF2196F3), fontWeight: FontWeight.bold, fontSize: 22)),
                    ],
                  ),
                ),
                const SizedBox(height: 30),
                // QR Code placeholder - using a more stable image or better handling
                Container(
                  width: 120,
                  height: 120,
                  decoration: BoxDecoration(
                  ...
                  child: Image.network(
                    'https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${bill.billCode}',
